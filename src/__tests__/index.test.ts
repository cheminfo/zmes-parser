...
test('parse test.zmes file', async () => {
  ...
  expect(record?.guid).toBe('f5e0a56f-3426-427a-91ca-731e963cc86b');
  expect(record?.createdDateTime).toBe('2026-02-25 08:33:16.9178437');
  expect(record?.group.id).toBe(1);
  expect(record?.group.guid).toBe('069ff74d-8d09-44c6-b89a-4acc9e973236');
});
...
test('scalar parameter values are correctly extracted', async () => {
  ...
  ).toBe('2026-02-25T08:33:17.2828726Z');

  // Int64 values
  expect(findParameter(children, 'Repeat')?.value).toBe(2);
...
test('actual instrument settings are extracted', async () => {
  ...
  expect(attenuator?.value).toBe(7);
  ...
  expect(numberOfRuns?.value).toBe(35);
});
...
test('sample settings contain sample name', async () => {
  ...
  expect(sampleName?.value).toBe('TEST XX230.A');
});
